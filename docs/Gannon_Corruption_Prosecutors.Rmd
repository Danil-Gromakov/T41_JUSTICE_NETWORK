...
## Save
````{r}
work <- work %>% dplyr::select(attorney, employer, start_date, end_date)
...
memb <- write.csv(memb_clean, paste0(here::here(), "/data/", "prosecutors_membership.csv"))
```
...
## Save
```{r}
full <- write.csv(full, paste0(here::here(), "/data/", "prosecutors_final.csv"))
```