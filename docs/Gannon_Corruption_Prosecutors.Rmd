...
ExPanDaR::prepare_missing_values_graph(df, ts_id = "date_of_appointment")
```
...
## School
```{r}
# To get counts per school we have to re-gather the split columns
tab <- df_wiki %>%
    tidyr::gather("id", "educated", educated_atLabel1, educated_atLabel2, educated_atLabel3) %>% 
    dplyr::group_by(educated) %>%
...
# Merge wikidata with primary
```{r, warning = FALSE}
df_full <- dplyr::left_join(df, df_wiki, by = c('wikidata_id' = 'id'))

# Only keep non-redundant rows
df_full <- df_full %>% dplyr::select(c("itemLabel", "district", "date_of_appointment", "barr_appointment", "sex_or_genderLabel", "date_of_birth", "place_of_birthLabel", "age", "educated_atLabel1", "educated_atLabel2", "educated_atLabel3"))
