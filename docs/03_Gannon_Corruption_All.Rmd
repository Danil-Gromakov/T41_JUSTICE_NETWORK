...
school_temp <- school_temp %>% dplyr::mutate(conservative = dplyr::if_else(score_scaled > 0, 1, 0)) %>%
  ...
  dplyr::rename(conservative_lawschool = conservative)
...
firm_temp <- firm_temp %>% dplyr::mutate(conservative = dplyr::if_else(score_scaled > 0, 1, 0)) %>%
  dplyr::select(person, conservative) %>%
  dplyr::rename(conservative_firm = conservative)
...
memb_temp <- df_noemploy %>% dplyr::select(person, american_center_for_law_and_justice, 
                            ...
                            dplyr::mutate(conservative_group = ifelse(american_center_for_law_and_justice == 1 | 
                                            ...
                                            "1", "")) %>%
                           dplyr::select(person, conservative_group)
...
employ_temp <- df %>% dplyr::select(person, employer_past) %>%
  dplyr::distinct() %>%
  dplyr::mutate(conservative_gov = ifelse(employer_past == "Idaho State Senate" |
                                          ...
                                          "1", "")) %>%
  dplyr::select(person, conservative_gov)
...
tomerge <- tomerge %>% dplyr::mutate(connection_conservativeagenda = ifelse(conservative_lawschool == 1 |
                                                                                conservative_firm == 1 |
                                                                                conservative_group == 1 |
                                                                                conservative_gov == 1,
                                                                              "1", "0")) %>%
  dplyr::select(person, connection_conservativeagenda) %>%
  dplyr::distinct()

tomerge$connection_conservativeagenda <- as.numeric(tomerge$connection_conservativeagenda)
tomerge$connection_conservativeagenda[is.na(tomerge$connection_conservativeagenda)] <- 0

# Merge duplicates
tomerge <- aggregate(tomerge$connection_conservativeagenda, by = list(tomerge$person), sum) %>% 
  dplyr::rename(person = Group.1, connection_conservativeagenda = x)

# Merge into master list
final <- dplyr::left_join(core, tomerge)

# Merge everything else into that now
df_noemploy <- dplyr::left_join(df_noemploy, final)
...
df_noemploy <- df_noemploy %>% dplyr::select("person", "position_current", "source",
                ...
                "bachelors", "masters", "doctorate", "law", "law_conservative_score", "law_rank",
                "barr_appointment", "political_affiliation", 
                "connection_trump", "connection_trumpally", "connection_conservativeagenda", "connection_careerprofessional", "connection_other",
                "american_center_for_law_and_justice", "christian_legal_society_bar_association", "federalist_society", "heritage_foundation", "national_rifle_association_tie", "republican_national_lawyers_association_tie")

# Fix NAs that should be 0's
df_noemploy$connection_trump[is.na(df_noemploy$connection_trump)] <- 0
df_noemploy$connection_trumpally[is.na(df_noemploy$connection_trumpally)] <- 0
df_noemploy$connection_conservativeagenda[is.na(df_noemploy$connection_conservativeagenda)] <- 0
df_noemploy$connection_careerprofessional[is.na(df_noemploy$connection_careerprofessional)] <- 0
df_noemploy$connection_other[is.na(df_noemploy$connection_other)] <- 0
```
