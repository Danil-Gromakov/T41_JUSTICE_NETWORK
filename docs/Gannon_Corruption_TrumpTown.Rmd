...
memb <- write.csv(memb_clean, paste0(here::here(), "/TrumpProsecutors_network/", "membership_hist.csv"))
```
...
```{r}
# Load
base <- read.csv(paste0(here::here(), "/TrumpProsecutors_network/", "wikidata_list.csv"))
employ <- read.csv(paste0(here::here(), "/TrumpProsecutors_network/", "employment_hist.csv"))
edu <- read.csv(paste0(here::here(), "/TrumpProsecutors_network/", "edu_hist.csv"))
memb <- read.csv(paste0(here::here(), "/TrumpProsecutors_network/", "membership_hist.csv"))
```
...
base <- base %>% dplyr::select(-c("X")) %>%
  dplyr::rename(person = itemLabel, sex = sex_or_genderLabel, birthplace = place_of_birthLabel)
...
full <- dplyr::left_join(employ, memb)
```

## Save
```{r}
full <- write.csv(full, paste0(here::here(), "/TrumpProsecutors_network/", "final_prosecutors.csv"))
```